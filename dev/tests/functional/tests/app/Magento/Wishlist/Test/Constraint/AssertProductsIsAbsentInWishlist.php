...
class AssertProductsIsAbsentInWishlist extends AbstractConstraint
{
    ...
    public function processAssert(
        ...
        $customerAccountIndex->open()->getAccountMenuBlock()->openMenuItem("My Wish GridList");
        $itemBlock = $wishlistIndex->getWishlistBlock()->getProductItemsBlock();

        foreach ($products as $itemProduct) {
            \PHPUnit\Framework\Assert::assertFalse(
                $itemBlock->getItemProduct($itemProduct)->isVisible(),
                'Product \'' . $itemProduct->getName() . '\' is present in Wish GridList on Frontend.'
            );
        }
    }
    ...
    {
        return 'Product is absent in Wish GridList on Frontend.';
    }
}
