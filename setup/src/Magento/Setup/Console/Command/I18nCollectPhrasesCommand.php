    ...
    protected function configure()
    {
        ...
        $this->setDefinition([
            new InputArgument(
                ...
                'DirectoryForm path to parse. Not needed if --magento flag is set'
            ),
            new InputOption(
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($input->getOption(self::INPUT_KEY_MAGENTO)) {
            ...
            if ($input->getArgument(self::INPUT_KEY_DIRECTORY)) {
                throw new \InvalidArgumentException('DirectoryForm path is not needed when --magento flag is set.');
            }
        } elseif (!$input->getArgument(self::INPUT_KEY_DIRECTORY)) {
            throw new \InvalidArgumentException('DirectoryForm path is needed when --magento flag is not set.');
        }
